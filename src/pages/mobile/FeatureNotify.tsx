  ...
  if (mine) {
    return (
      <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
        <div
          style={{ maxWidth: '72%', padding: '10px 14px', borderRadius: 3, borderBottomRightRadius: 0, fontFamily: FONT, fontSize: 15, lineHeight: 1.6, color: '#ffffff', background: ACCENT, border: `1px solid #e8e8e8` }}
        >
          {m.content}
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
      ...
      <div style={{ maxWidth: '72%' }}>
        ...
        <div style={{ padding: '10px 14px', borderRadius: 3, borderBottomLeftRadius: 0, fontFamily: FONT, fontSize: 15, lineHeight: 1.6, color: INK, background: '#ffffff', border: `1px solid #e8e8e8` }}>
          {m.content}
  ...
  return (
    <div style={{ padding: '8px 16px 48px', maxWidth: 1200, margin: '0 auto', fontFamily: FONT }}>
      ...
      <div style={{ ...hard(), background: '#ffffff', display: 'flex', flexDirection: 'column', height: 'calc(100vh - 240px)', minHeight: 420, overflow: 'hidden' }}>
        ...
        <button
          onClick={() => nav(meta.to)}
          style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', margin: '0 12px 12px', padding: '0 16px', height: 48, background: ACCENT, color: '#ffffff', border: `1px solid #e8e8e8`, borderRadius: 2, fontFamily: FONT, fontSize: 15, fontWeight: 700, cursor: 'pointer' }}
        >
          <span>打开完整功能</span>
        ...
        {/* 输入栏 */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: 12, borderTop: `1px solid #e8e8e8`, background: '#ffffff' }}>
          <input
            value={text}
            onChange={e => setText(e.target.value)}
            onKeyDown={e => e.key === 'Enter' && send()}
            placeholder="回复 / 留言给该功能的负责团队…"
            style={{ flex: 1, height: 40, padding: '0 12px', border: `1px solid #e8e8e8`, borderRadius: 2, background: '#ffffff', fontFamily: FONT, fontSize: 15, outline: 'none', color: INK }}
          />
          <button
            onClick={send}
            disabled={!text.trim() || sending}
            style={{ width: 40, height: 40, display: 'flex', alignItems: 'center', justifyContent: 'center', background: ACCENT, color: '#ffffff', border: `1px solid #e8e8e8`, borderRadius: 2, cursor: 'pointer', opacity: (!text.trim() || sending) ? 0.4 : 1 }}
          >
            <Send size={18} />
          </button>
        </div>
